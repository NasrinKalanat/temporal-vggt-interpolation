"""Train TemporalVGGTv1 with LOTO cross-validation.
...
(protocol, crop, [test_date]) fold:
  ...
  - Trains TemporalVGGTv1 with AdamW + cosine-warmup LR
...
def build_model(cfg: dict[str, Any], device: str, feature_cache=None) -> torch.nn.Module:
    ...
    # Freeze point head if configured: skip forward + free params from memory
    if cfg.get("freeze_point_head", False):
        ...
        if hasattr(model, "point_head"):
            ...
            model.point_head = None  # free memory
...
def _collate(samples: list) -> dict:
    ...
    for k in samples[0]:
        ...
        if any(v is None for v in vals):
            ...
            # List of tensors (e.g. cached layers): stack per-layer across batch
...
def warmup_feature_cache_sharded(
    model: torch.nn.Module,
    dataset: TemporalTripletDataset,
    device: str,
    image_preprocess_mode: str,
    ...
    """Sharded warmup: each rank processes a slice of missing cache entries."""
    cache = getattr(model, "feature_cache", None)
    if cache is None or not hasattr(dataset, "index"):
        return
    ...
    all_keys = sorted(missing.keys())
    my_keys = all_keys[rank::world_size]

    if not my_keys:
        return

    log(f"[rank {rank}] Feature cache warmup: {len(my_keys)}/{len(all_keys)} entries")
    model.eval()
    autocast_ctx = (
        torch.autocast("cuda", dtype=torch.bfloat16)
        if device.startswith("cuda") else torch.autocast("cpu", enabled=False)
    ...
    with torch.no_grad(), autocast_ctx:
        for key in tqdm(my_keys, desc=f"cache warmup [rank {rank}]", unit="view", disable=(rank != 0)):
            if cache.exists(key):
                continue
            date_dir = missing[key]
            images = _load_images(date_dir, image_preprocess_mode).unsqueeze(0).to(device)
            B, S = 1, images.shape[1]
            feats = model._run_vggt_endpoint(images, B, S)
            cache.put(key, feats)
        ...
        with _autocast(device):
            if cfg.get("gradient_checkpointing"):
                outputs = torch.utils.checkpoint.checkpoint(
                    lambda: model(batch), use_reentrant=False
                )
            else:
                outputs = model(batch)
        loss, loss_dict = compute_loss(outputs, batch, device, cfg, model=model)
        loss.backward()
        if grad_clip > 0:
...
def eval_epoch(
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            with _autocast(device):
                outputs = model(batch)
            _, loss_dict = compute_loss(outputs, batch, device, cfg, model=model)
            _accumulate(totals, loss_dict)
            n += 1
            pbar.set_postfix(loss=f"{loss_dict.get('loss_pointmap', 0):.4f}")
    return {k: v / max(n, 1) for k, v in totals.items()}
...
def _train_fold_inner(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        train_metrics = train_epoch(model, train_loader, optimizer, device, cfg, grad_clip)
